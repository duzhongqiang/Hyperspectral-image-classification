function [LBP_fea lbp_img] = LBP_feature_global(Data, radius, num_point, mapping, W0)
...
X(W+1:m+W, W+1:n+W, :) = Data;
X(W+1:m+W, 1:W, :) = Data(:, W:-1:1, :);
X(W+1:m+W, n+W+1:n+2*W, :) = Data(:, n:-1:n-(W-1), :);
X(1:W, :, :) = X(2*W:-1:(W+1), :, :);
...
for i = 1:d
    I = X(:,:,i);
    I = I./max(I(:));
    lbp_img = lbp_HSI(I,radius,num_point,mapping,'h');    
    hist_fea = hist_lbp_HSI(lbp_img, mapping, W0, 'h');
...
function result = lbp_HSI(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.3.3
...
function lbp_hist_fea = hist_lbp_HSI(result, mapping, W, mode)
